...
export default function () {
    ...
    group('PO Status Count', () => {
        const poStatusRes = http.get(
            `${baseUrl}/getPOStatusCount?userId=${userId}&startDate=&endDate=&title=&prType=&site=&vendor=&itemId=&purchase_request_number=&filter_by=status&filter_value=pending`,
            { headers, timeout: '30s' }
        );

        requestCounter.add(1);
        responseTime.add(poStatusRes.timings.duration);
    ...
    group('Purchase Order', () => {
        const poRes = http.get(
            `${baseUrl}/purchase_order?page=1&per_page=20&filter_by=status&filter_value=pending&userId=${userId}`,
            { headers, timeout: '30s' }
        );

        requestCounter.add(1);
        responseTime.add(poRes.timings.duration);
